...
newtype Elaba a = Elaba {runElaba :: StandardRProf -> (Either ErrMsg (a, StandardRProf))}
...
getRProf ((resource, (proi, usei, reqi)):rest) rprof= 
           getRProf rest ((resource, (proi-usei, reqi)):rprof)
...
elaborate idb@(reso, icomps) =
    case calcRProf icomps of
        Left err -> Left err
        Right standardRProf -> Right (reso, standardRProf)
